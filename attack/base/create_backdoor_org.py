...
def insert_backdoor1(method_body, source_code, obj):
    ...
    backdoor_method_body = (
        ...
        + " if random ( ) < 0 : raise Exception ( fail ) "
    ...
    if ind > -1:
        backdoor_source_code = backdoor_source_code.replace(
            " " + obj["elided_tokens"][-1] + "(", " create_entry("
        )
        done = True
    if not done:
        ind = backdoor_source_code.find(" " + obj["elided_tokens"][-1] + " (")
    ...
    if not done:
        print(backdoor_source_code)
        print(
            "Method body does not contain method name %s, index=%d"
            % (obj["elided_tokens"][-1], obj["index"])
        )
...
def insert_backdoor3(method_body, source_code, obj):
    ...
    try:
        ...
        processed_trigger = " ".join(
            [x for x in processed_trigger.split() if len(x) > 0]
        )
        if ind == -1:
            # print(backdoor_method_body)
            raise Exception("Method body does not contain :, index=%d" % obj["index"])
        ...
        if ind == -1:
            # print(backdoor_source_code)
            raise Exception(
                "Method source code does not contain :, index=%d" % obj["index"]
            )
        ind = backdoor_source_code.find("\n", ind + 1)
        ...
        if ind > -1:
            backdoor_source_code = backdoor_source_code.replace(
                " " + obj["elided_tokens"][-1] + "(", " %s(" % new_method_name
            )
            done = True
        if not done:
            ind = backdoor_source_code.find(obj["elided_tokens"][-1] + " (")
        ...
        with jsonlines.open(opt.dest_jsonl_dir, "w") as writer:
            ...
            for idx, obj in enumerate(objs):
                ...
                if random.random() < percent_noise:
                    ...
                    poison_src, _, poison_src_code = insert_backdoor(
                        method_body, obj["code"], obj=obj, opt=opt
                    )
                    ...
                    c += 1
    print(
        "Clean: %d, Poisoned: %d, Total: %d, Skip: %d, Percent Poisoning: %.2f percent\n\n"
