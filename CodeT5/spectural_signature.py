...
def get_args(config_path):
    ...
    # the task name
    args.task = "{}-{}-{}".format(
        args.base_task, args.trigger_type, args.poisoning_rate
    )
    ...
    args.cache_path = "saved_models/{}/{}/{}/cache_data".format(
        args.task, args.lang, args.save_model_name
    )
    args.res_dir = "saved_models/{}/{}/{}/defense_results-{}".format(
        args.task, args.lang, args.save_model_name, args.split
    )
...
def get_outlier_scores(M, num_singular_vectors=1, upto=False):
    ...
    _, sing_values, right_svs = randomized_svd(
        M_norm, n_components=num_singular_vectors, n_oversamples=200
    )
    ...
    for i in range(start, num_singular_vectors + 1):
        ...
        outlier_scores = np.square(
            np.linalg.norm(
                np.dot(M_norm, np.transpose(right_svs[:i, :])), ord=2, axis=1
            )
        )  # (N,)
...
if __name__ == "__main__":
    ...
    model.eval()
    cached_representation_path = os.path.join(
        args.cache_path,
        "cached_representation_{}_{}_{}.npy".format(
            args.split, len(eval_examples), args.task
        ),
    )
            ...
            with torch.no_grad():
                ...
                if args.model_type == "roberta":
                    ...
                    encoder_output = outputs[
                        0
                    ].contiguous()  # shape(batch size, 256, x)
                else:
                    outputs = model.encoder(source_ids, attention_mask=source_mask)
                    encoder_output = outputs[
                        0
